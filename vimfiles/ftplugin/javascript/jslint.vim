...
if !exists(":JSLintToggle")
  ...
  "command JSLintToggle exec 
  "":let b:jslint_disabled = exists('b:jslint_disabled') ? b:jslint_disabled ? 0 : 1 : 1" |
...
if !exists("*s:JSLintToggle")
  function s:JSLintToggle()
      let b:jslint_disabled = exists('b:jslint_disabled') ? b:jslint_disabled ? 0 : 1 : 1
      echo 'JSLint ' . ['enabled', 'disabled'][b:jslint_disabled] . '.'
  
  	if b:jslint_disabled == 0
  		let g:JSLintHighlightErrorLine = 1
  		call s:JSLint()
  	else
  		let g:JSLintHighlightErrorLine = 0
...
"au BufRead,BufNewFile *.js noremap <F4> :JSLintToggle<CR> 
